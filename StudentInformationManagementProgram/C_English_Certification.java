...
public class C_English_Certification extends JFrame 
{
	Data_Base D;

	public C_English_Certification(Data_Base D)
	{
		this.D = D;
		String s = "";
		int p=0, n=0;
		...
		for(p=0; p<D.total; p++)
		{
			if(D.now_id.equals(D.R_ID.get(p)) && D.RECOGNITION.get(p).equals("English Certification"))
			{
				s = D.R_CHECK.get(p);
				break;
			}
		...
		else if(D.RECOGNITION1.get(n).equals("3"))
		{
		...
		b2.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				if(b.getText().equals("Process"))
				{
					...
					for(i=0; i<D.total; i++)
					{
						if(D.now_id.equals(D.R_ID.get(i)) && D.RECOGNITION.get(i).equals("English Certification") 
