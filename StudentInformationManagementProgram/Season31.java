...
public class Season31 extends JFrame
{
	Data_Base D;

	Season31(Data_Base D)
	{
		this.D = D;
		int i=0, p=0;
		int num=100;

		setTitle("Score of Season31");
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		Container c = getContentPane();
		c.setLayout(null);

		JLabel la = new JLabel("<< Score of Season31 >>");
		...
		for(i=0; i<D.L_total; i++)
		{
			...
			{
				if(D.STUDENT.get(i).print(p).equals(D.now_id) && D.SEASON.get(i).equals("31"))
				{
